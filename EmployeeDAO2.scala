...
class EmployeeDAO2 {
  val db = Database.forURL("jdbc:mysql://localhost:3306/mydb?user=root&password=root", driver = "com.mysql.jdbc.Driver")
  ...
  def findEmployeeById(employeeId: Int): Future[Seq[EmployeeRow]] = {
    ...
    var comp = for (emprow <- employees if emprow.employeeId === employeeId) yield emprow
  ...
  def getAllEmployeesSelectedColumnsUsingComp(): Future[Seq[(Int, String, Double)]] = {
    var comp = for (emprow <- employees) yield (emprow.employeeId, emprow.employeeName, emprow.salary)
  ...
  def getAllEmployeesSelectedColumnsUsingLambads(): Future[Seq[(Int, String, Double)]] = {
    var comp = employees.map(row=>(row.employeeId,row.employeeName,row.salary))
  ...
  //Sorting and
  def getAllEmployeesSalBetweenAndOrderBy(lowerBound: Double, upperBound: Double, order: String): Future[Seq[EmployeeRow]] = {

    if (order == "desc") {
      var action = employees.
        filter(emp => emp.salary >= lowerBound && emp.salary <= upperBound).sortBy(emp => emp.salary.desc.nullsFirst)
        .result
      action.statements.foreach(println(_))
      db.run(action)
    } else {
      db.run(employees.
        filter(emp => emp.salary >= lowerBound && emp.salary <= upperBound).sortBy(emp => emp.salary.asc.nullsLast)
        .result)
    }
  }
